...
import com.anjieyang.uooj.crawler.language.LanguageContext;
...
import com.anjieyang.uooj.dao.problem.ProblemLanguageEntityService;
...
//      checkAllLanguageUpdate();
    ...
    private void upsertHOJLanguageV2() {
        ...
        if (countRuby == 0) {
            ...
            if (!isOk) {
                log.error("[Init System Config] [UOOJ] Failed to add new language [{}]! Please check whether the language table corresponding to the database has the language!", "Ruby");
            }
        }
        ...
        if (countRust == 0) {
            ...
            if (!isOk) {
                log.error("[Init System Config] [UOOJ] Failed to add new language [{}]! Please check whether the language table corresponding to the database has the language!", "Rust");
            }
        }
    }
    ...
    private void upsertHOJLanguage(String... languageList) {
        ...
        for (String language : languageList) {
            ...
            if (count == 0) {
                ...
                if (!isOk) {
                    log.error("[Init System Config] [UOOJ] Failed to add new language [{}]! Please check whether the language table corresponding to the database has the language!", language);
                }
            }
        }
    }
