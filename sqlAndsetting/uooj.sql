...
CREATE DATABASE IF NOT EXISTS`uooj` DEFAULT CHARACTER SET utf8;
...
CREATE TABLE `announcement` (
  ...
  `status` int(11) DEFAULT '0' COMMENT '0 is visible，and 1 is invisible',
...
CREATE TABLE `auth` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(100) DEFAULT NULL COMMENT 'Permission Name',
  `permission` varchar(100) DEFAULT NULL COMMENT 'Permission String',
  `status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '0 for enabled, 1 for disabled',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
...
CREATE TABLE `comment` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `cid` bigint(20) unsigned DEFAULT NULL COMMENT 'null indicates no reference to a competition',
  `did` int(11) DEFAULT NULL COMMENT 'null indicates no reference to a discussion',
  `content` longtext COMMENT 'Comment Content',
  `from_uid` varchar(32) NOT NULL COMMENT 'Commenter ID',
  `from_name` varchar(255) DEFAULT NULL COMMENT 'Commenter Username',
  `from_avatar` varchar(255) DEFAULT NULL COMMENT 'Commenter Group Avatar URL',
  `from_role` varchar(20) DEFAULT NULL COMMENT 'Commenter Role',
  `like_num` int(11) DEFAULT '0' COMMENT 'Number of Likes',
  `status` int(11) DEFAULT '0' COMMENT 'Whether to ban or logically delete this comment',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `uid` (`from_uid`),
...
CREATE TABLE `contest` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `uid` varchar(32) NOT NULL COMMENT 'ID of the Competition Creator',
  `author` varchar(255) DEFAULT NULL COMMENT 'Username of the Competition Creator',
  `title` varchar(255) DEFAULT NULL COMMENT 'Competition Title',
  `type` int(11) NOT NULL DEFAULT '0' COMMENT '0 for ACM-style, 1 for Score-based Competition',
  `description` longtext COMMENT 'Competition Description',
  `source` int(11) DEFAULT '0' COMMENT 'Competition Source, 0 for original, competition ID for clone',
  `auth` int(11) NOT NULL COMMENT '0 for Public Competition, 1 for Private Competition (requires password for access), 2 for Protected Competition (submission requires password)',
  `pwd` varchar(255) DEFAULT NULL COMMENT 'Competition Password',
  `start_time` datetime DEFAULT NULL COMMENT 'Start Time',
  `end_time` datetime DEFAULT NULL COMMENT 'End Time',
  `duration` bigint(20) DEFAULT NULL COMMENT 'Competition Duration (in seconds)',
  `seal_rank` tinyint(1) DEFAULT '0' COMMENT 'Whether to enable Rank Sealing',
  `seal_rank_time` datetime DEFAULT NULL COMMENT 'Rank Sealing Start Time, continues until the end of the competition without leaderboard updates',
  `auto_real_rank` tinyint(1) DEFAULT '1' COMMENT 'Whether to automatically lift Rank Sealing at the end of the competition, converting to real leaderboard',
  `status` int(11) DEFAULT NULL COMMENT '-1 for Not Started, 0 for In Progress, 1 for Ended',
  `visible` tinyint(1) DEFAULT '1' COMMENT 'Whether it is visible',
  `open_print` tinyint(1) DEFAULT '0' COMMENT 'Whether to enable printing functionality',
  `open_account_limit` tinyint(1) DEFAULT '0' COMMENT 'Whether to enable account restrictions',
  `account_limit_rule` mediumtext COMMENT 'Account Restriction Rules',
  `rank_show_name` varchar(20) DEFAULT 'username' COMMENT 'Leaderboard Display (username, nickname, realname)',
  `open_rank` tinyint(1) DEFAULT '0' COMMENT 'Whether to open the competition leaderboard',
  `star_account` mediumtext COMMENT 'Starred User List',
  `oi_rank_score_type` varchar(255) DEFAULT 'Recent' COMMENT 'OI Leaderboard Scoring Type (Recent, Highest)',
  `is_group` tinyint(1) DEFAULT '0' COMMENT 'Whether it is a group competition',
  `gid` bigint(20) unsigned DEFAULT NULL,
  `award_type` int(11) DEFAULT '0' COMMENT 'Award Type: 0 (Not Set), 1 (Set by Percentage), 2 (Set by Number)',
  `award_config` text DEFAULT NULL COMMENT 'Award Configuration JSON',
  `allow_end_submit` tinyint(1) DEFAULT '0' COMMENT 'Whether to allow submissions after the competition has ended',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`,`uid`),
  KEY `uid` (`uid`),
...
CREATE TABLE `contest_announcement` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `aid` bigint(20) unsigned NOT NULL COMMENT 'Announcement ID',
  `cid` bigint(20) unsigned NOT NULL COMMENT 'Competition ID',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `contest_explanation` (
  ...
  `uid` varchar(32) NOT NULL COMMENT 'Publisher (must be the competition creator or super administrator)',
  `content` longtext COMMENT 'Content (supports markdown)',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `uid` (`uid`),
...
CREATE TABLE `contest_problem` (
  ...
  `display_id` varchar(255) NOT NULL COMMENT 'Sequential ID of the Problem within the Competition',
  `cid` bigint(20) unsigned NOT NULL COMMENT 'Competition ID',
  `pid` bigint(20) unsigned NOT NULL COMMENT 'Problem ID',
  `display_title` varchar(255) NOT NULL COMMENT 'Title of the Problem within the Competition (default is the original name)',
  `color` varchar(255) DEFAULT NULL COMMENT 'Balloon Color',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `contest_record` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Primary Key',
  `cid` bigint(20) unsigned DEFAULT NULL COMMENT 'Competition ID',
  `uid` varchar(255) NOT NULL COMMENT 'User ID',
  `pid` bigint(20) unsigned DEFAULT NULL COMMENT 'Problem ID',
  `cpid` bigint(20) unsigned DEFAULT NULL COMMENT 'ID of the Problem within the Competition',
  `username` varchar(255) DEFAULT NULL COMMENT 'Username',
  `realname` varchar(255) DEFAULT NULL COMMENT 'Real Name',
  `display_id` varchar(255) DEFAULT NULL COMMENT 'Displayed ID within the Competition',
  `submit_id` bigint(20) unsigned NOT NULL COMMENT 'Submission ID, used for possible re-judging',
  `status` int(11) DEFAULT NULL COMMENT 'Submission Result, 0 indicates not AC with no penalty time, 1 indicates AC, -1 indicates not AC with penalty time',
  `submit_time` datetime NOT NULL COMMENT 'Exact Submission Time',
  `time` bigint(20) unsigned DEFAULT NULL COMMENT 'Submission Time, calculated as submission time minus competition start time',
  `score` int(11) DEFAULT NULL COMMENT 'Score for OI Competition',
  `use_time` int(11) DEFAULT NULL COMMENT 'Execution Time',
  `first_blood` tinyint(1) DEFAULT '0' COMMENT 'Whether it is the first blood AC (deprecated)',
  `checked` tinyint(1) DEFAULT '0' COMMENT 'Whether the AC has been verified',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `contest_register` (
  ...
  `status` int(11) DEFAULT '0' COMMENT 'Default is 0 for normal, 1 for invalid.',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `contest_score` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `cid` bigint(20) unsigned NOT NULL,
  `last` int(11) DEFAULT NULL COMMENT 'Score before the competition',
  `change` int(11) DEFAULT NULL COMMENT 'Score change during the competition',
  `now` int(11) DEFAULT NULL COMMENT 'Current score',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`,`cid`),
...
CREATE TABLE `discussion` (
  ...
  `title` varchar(255) DEFAULT NULL COMMENT 'Discussion title',
  `description` varchar(255) DEFAULT NULL COMMENT 'Discussion description',
  `content` longtext COMMENT 'Discussion content',
  `pid` varchar(255) DEFAULT NULL COMMENT 'Associated problem ID',
  `uid` varchar(32) NOT NULL COMMENT 'Publisher ID',
  `author` varchar(255) NOT NULL COMMENT 'Publisher username',
  `avatar` varchar(255) DEFAULT NULL COMMENT 'Avatar of the discussion author',
  `role` varchar(25) DEFAULT 'user' COMMENT 'Publisher role',
  `view_num` int(11) DEFAULT '0' COMMENT 'Number of views',
  `like_num` int(11) DEFAULT '0' COMMENT 'Number of likes',
  `top_priority` tinyint(1) DEFAULT '0' COMMENT 'Priority, whether it is pinned',
  `comment_num` int(11) DEFAULT '0' COMMENT 'Number of comments',
  `status` int(1) DEFAULT '0' COMMENT 'Whether the discussion is banned',
  `gid` bigint(20) unsigned DEFAULT NULL,
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `discussion_report` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `did` int(11) DEFAULT NULL COMMENT 'Discussion ID',
  `reporter` varchar(255) DEFAULT NULL COMMENT 'Username of the reporter',
  `content` varchar(255) NOT NULL COMMENT 'Reported content',
  `status` tinyint(1) DEFAULT '0' COMMENT 'Whether it has been read',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `did` (`did`),
...
CREATE TABLE `problem` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `problem_id` varchar(255) NOT NULL COMMENT '问题的自定义ID 例如（UOOJ-1000）',
